...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
    ({ className, type, hint, error, leadingIcon, trailingIcon, ...props }, ref) => {
        ...
        if (leadingIcon) {
            LEADING_ICON = React.cloneElement(leadingIcon, {
                color: error ? colors.rose[500] : isFocused ? colors.alpha['00'] : colors.alpha[500]
            })
        }

        if (trailingIcon) {
            TRAILING_ICON = React.cloneElement(trailingIcon, {
                color: error ? "red" : isFocused ? "var(--color-alpha-00)" : "var(--color-alpha-500)"
            })
        }
            ...
            <div className="flex flex-col">
                <div className="relative">
                    ...
                    <input
                        ...
                        className={cn(
                            "peer",
                            "flex h-14 w-full px-4 py-3 justify-between items-center rounded-lg border border-solid bg-alpha-900",
                            {
                ...
                {error 
                    ...
                    : hint ? <p className="text-sm text-alpha-500 mt-1">{hint}</p> 
                    : null
                }
            </div>
        );
